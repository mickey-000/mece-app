...
def _parse_json(text):
    """モデル出力からJSON部分を取り出して辞書にする。失敗時は {"_raw": text}。"""
    import json
    t = str(text).strip()
...
def mece_feedback(problem, a1, a2, a3, a4):
    """受講者の①〜④の回答を講評し、辞書で返す（模範解答は構造化データ）。"""
...
出力は必ず次のJSON形式のみ。前後に説明やコードブロック記号、HTMLタグは一切付けないこと。
各値はプレーンな日本語の文章（記号「|」やタグは使わない）。

{{
  "good": "良かった点を具体的にほめる文章（未記入が多ければ、考え方の入口をやさしく示す）",
  "improve": "改善ポイントを前向きに、具体例を挙げて示す文章",
  "model_answer": {{
    "事象": "模範解答（中立的な事実のみ）",
    "問題": "模範解答（あるべき姿とのギャップ）",
    "課題": "模範解答（具体的アクション。無ければ「該当なし」）",
    "その他": "模範解答（感情・主観・決めつけ等）"
  }},
  "advice": "発言者の論理の飛躍や思考バイアスへの、共感的なひとことアドバイス"
}}"""
    return _parse_json(_oneshot(MECE_ROLE, user))
